 ...
 * @param {*} svgColor 要在此 <svg> 套用的顏色
 * @returns 返回一段 HTML 結構，該結構包含 svg 圖片、文字內容
 */
export function createEmptyMsg(msgText, svgTag, svgColor) {
  // @ svg 中預設的主色調為 '#6c63ff'，利用 replace() 將 svgTag 中的該色調替換成指定的顏色(svgColor)
  while (svgTag.indexOf("#6c63ff") !== -1) {
    svgTag = svgTag.replace("#6c63ff", svgColor);
    svgTag = svgTag.replace("#888", svgColor);
  }

  return `
    <div class="empty-msg">
      ${svgTag}
      <p class="empty-msg__text">
        ${msgText}
      </p>
    </div>
  `;
